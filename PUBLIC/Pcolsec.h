...
class CPSecondary : public CProtocol
{
	public:
		...
		WORD GetCosNum(WORD HeadPtr,WORD TailPtr,BYTE bSoeBufLen = SOE_BUFF_NUM);
		WORD ReadSoeFromFlash(WORD *pHeadPtr,WORD TailPtr,BYTE bSoeMaxNum,BYTE *pSoeBuf);
		WORD ReadSoeFromFlash_FILE(WORD *pHeadPtr,WORD TailPtr,BYTE bSoeMaxNum,BYTE *pSoeBuf,WORD *pNum_Total);
...
//			WORD ReadRangeSYXBit(WORD wBeginNo,WORD wNum,BYTE *buf);
		...
		WORD SearchChangeYC(WORD wNum, WORD wChgYcNumMax,VDBYCF *pBuf);
		void CheckChangeYC();
...
public:
		...
		BYTE m_PaWaitflag_lubo;//ON表示启动重发计时 TRUE表示重发计时到 OFF表示重发结束(重发次数到或收到回复)
		WORD m_PaWaitCt_lubo;//du参数时重发计时
		WORD m_TxNum_lubo;//重传次数
		BYTE  m_gprsSendBuf[256];//保存需要重传的报文
		WORD  m_gprsSendLen;//需要重传的报文长度
       
        UINT m_SFeiHeadPtr;
        UINT m_SFeiHeadPtrBk;
};
