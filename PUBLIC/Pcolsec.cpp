...
{
        m_pEqpInfo[0].wLinkAddress =  g_gRunPara[RP_COMM_ADDR];
        m_pEqpInfo[0].wAddress =  g_gRunPara[RP_COMM_ADDR];
        m_pEqpInfo[0].wSYXNum     = g_ucYxTransNum-g_ucDYxTransNum;
        m_pEqpInfo[0].wDYXNum     = g_ucDYxTransNum;
...
WORD CPSecondary::ReadSoeFromFlash(WORD *pHeadPtr,WORD TailPtr,BYTE bSoeMaxNum,BYTE *pSoeBuf)
{
    ...
    {
        Sst26vf064b_Read(FADDR_SOE_START +(long)(*pHeadPtr)*FSOE_LEN,bSoeBufTmp, SOE_DATA_LEN);
...
WORD CPSecondary::ReadSoeFromFlash_FILE(WORD *pHeadPtr,WORD TailPtr,BYTE bSoeMaxNum,BYTE *pSoeBuf,WORD *pNum_Total)
{
    int RecNum = 0;
    BYTE bSoeBufTmp[SOE_DATA_LEN] = {0};
    while((*pHeadPtr) != TailPtr)
	...
	(*pNum_Total)++;//防止FLASH出错时，总个数对不上的情况
        (*pHeadPtr)++;
        (*pHeadPtr) &= (FSOE_TOLNUM - 1);
        if(RecNum >= bSoeMaxNum) 
            return RecNum;
        ...
        if((g_ucYCAddr[i] - 1 != RM_BAT_U)
           ...
           /*&& (g_ucYCAddr[i] - 1 != RM_BATF_U)*/)
         {
              ...
              if(wCurYcVal!=(short)INVALIDYC)
              {
                ...
                if((((wYcDval > g_gRunPara[RP_YCLIMIT])&&(g_ucYCAddr[i]<RM_TEMP_START))&&(((INT32U)wYcDval*100) > ((INT32U)g_gRunPara[RP_YCCAP]*wOldYcVal)))||((g_ucYCAddr[i]>=RM_TEMP_START)&&(wYcDval > g_gRunPara[RP_TELIMIT])&&(g_ucYCAddr[i]!=255)))//m_pEqpInfo[m_wEqpNo].YcPara[i]
                 {
                    m_SendChgYcFlag = 1;
...
/***************************************************************
  ...
  返回：有变化的遥测个数 （目前为总个数）
***************************************************************/
WORD CPSecondary::SearchChangeYC(WORD wNum,WORD wChgYcNumMax,VDBYCF *pBuf)
{
   // #define FRM_CHGYCNUM  30//46  //每帧传输变化遥测个数  由于添加浮点数，全部改成30
    WORD i=0,wRtNum = 0;
    short  wCurYcVal,wOldYcVal;
  WORD wYcDval;
        WORD  CurYc[IEC_YC_NUM];
