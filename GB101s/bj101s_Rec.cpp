  ...
  else//激活
  {
      ...
      for( BYTE i =0; i<bQos;i+=2 )
      {
        ...
        else if((wInfoAddr == 0x5008)||(wInfoAddr == 0x5009)||(wInfoAddr == 0x500B)||(wInfoAddr == 0x500D)||(wInfoAddr == 0x500E)||(wInfoAddr == 0x500F)||(wInfoAddr == 0x5011)||(wInfoAddr == 0x5014))
        {
			...
			g_RWFiInf.Para_guiyue =OFF;//回复哪个规约版本的参数
        }
      }
...
void CBJ101S:: ReadFileDataUlog(WORD FileName,BYTE section_current,BYTE flag)
{
 ...
 // WORD log_HeadLeng;
  ...
  else if(mlog_recorded.log_count_flag == ON)
  {
    ...
    wCurPos = mwavelog_New -1- mlog_recorded.log_Ptr;
      if(wCurPos < 0)
...
/*void CBJ101S::ReadFileDataExv(WORD FileName,BYTE section_current,BYTE flag)
{
 ...
 // BYTE YC_Type =9;//归一化? 
  ...
  if(mfixpt_recorded.fixpt_count_flag == OFF)
  {
    ...
    sprintf((char *)ch,"%s\r\n%s,%04d-%02d-%02d,%02d,%02d\r\n",pdat_name,pDevID,byLoadDa[2]+2000,byLoadDa[3],byLoadDa[4],1,object_addr_length);//1表示条数，极值文件一天就一条
    FixPt_Sum =0;
    for(n = 0; n < strlen(ch); n++)
    {
      m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ] = ch[n];
  ...
  else if(mfixpt_recorded.fixpt_count_flag == ON)
  {
        ...
        if(g_ucYCAddr[i]!=255)
        {
            ...
            {         
                ...
                if(m_guiyuepara.yctype == M_ME_NC)  //短浮点数
                {
                    ...
                    offset +=sprintf(ch + offset,"%02x%02x,%.2f,",infoaddrh,infoaddrl,YCValF);
                }
                else
                {
                    WORD YCValW = (WORD)(YcValue * GetYcCoef(YcIndex));             
                    offset +=sprintf(ch + offset,"%02x%02x,%d,",infoaddrh,infoaddrl,YCValW);
                }
                //offset +=sprintf(ch + offset,"%02x%02x,%d,",infoaddrh,infoaddrl,MAKEWORD(byLoadDa[8+YcIndex*2],byLoadDa[9+YcIndex*2]));
            }
            if (strlen(ch) >= 180)
            {
     ...
     if(mfixpt_recorded.fixpt_count >= g_ucYcTransNum && strlen(ch) > 0)//每一节后加换行
     {//判断ch数组中有无需要传输的字符，若无则不加换号
        ch[offset -1]='\r';
        ch[offset]='\n';
     }
     if(mfixpt_recorded.fixpt_Ptr ==0)
     {
        ...
        mfixpt_recorded.fixpt_Curren_count = (fix_YC_Num *10 + 1)/180 +1 ;
  ...
  else if(mfixpt_recorded.fixpt_count_flag == ON)
  {
    ...
    if(mfixpt_recorded.fixpt_Ptr ==0)
    {
	  ...
	  if(g_ucPara101[IECP_YC_TYPE] == 2) //短浮点数
            mfixpt_recorded.fixpt_Curren_count = (fix_YC_Num *14 )/strlen(ch) +1 ;//160
      else
           mfixpt_recorded.fixpt_Curren_count= (fix_YC_Num *11 )/strlen(ch) +1 ;
  ...
  if(g_gRunPara[RP_LUBO_CYCLE] ==0)//(||(offset_time_Max ==0))//对时没对上
    ...
    /* mid_Value = MAKEWORD(datBuff[14],datBuff[15]);
     u0 = (long)(g_gRunPara[RP_VOLTAGE_V0]) * mid_Value/1000;*/
          ...
          {
            sprintf((char *)ch,"%u,%lu,%d,%d,%d,%d,%d,%d,%d\r\n",(unsigned int)DatNum,(unsigned long)mSendDATNum*250,ua,ub,uc,ia,ib,ic,i0);//,MAKEWORD(datBuff[8],datBuff[9]),MAKEWORD(datBuff[10],datBuff[11]),
                                 //MAKEWORD(datBuff[12],datBuff[13]),MAKEWORD(datBuff[14],datBuff[15]));//每个周期80个点，采样周期250微妙
          }
          #else
          //int I_Value = 1414;
  ...
  /* mid_Value = MAKEWORD(datBuff[16],datBuff[17]);
    u0 = (long)(g_gRunPara[RP_VOLTAGE_V0]) * mid_Value/1000;
        ...
        datBuff_BK[26]  = datBuff[18];; 
            ...
            {
              sprintf((char *)ch,"%u,%lu,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",(unsigned int)DatNum,(unsigned long)mSendDATNum*250,ia,ib,
...
void CBJ101S::Send_ReadFile_Confirm(WORD File_ID,BYTE File_TYPE,WORD InfoAddr,unsigned char *pRxBuf,unsigned char length)
{
      ...
      if(YcIndex <RM_1A_LUBO )//RM_1A_CABLE_T(YcIndex >= RM_BAT_U) && ((RM_1A_LOAD_I + RM_LOADI_NUM))
        ...
        YC_NUM++;
		 ...
		 if(m_guiyuepara.yctype == M_ME_NC)  //短浮点数
		 {
		 	...
		 	m_Fixpt_SECTION_Leng = 24 + 43+28+YC_NUM*14;//第一行报文头24个字节第二行报文头43
         	//m_Fixpt_TOTAL_Leng =17+((DWORD)(18+YC_NUM*10)* LoadDay);//文件头17个字节+节头+YC的个数
        	m_Fixpt_TOTAL_Leng = 24+43+((DWORD)(28+YC_NUM*14)* LoadDay);
		}
		else 
		{
			m_Fixpt_SECTION_Leng = 24 + 43+28+YC_NUM*11;//第一行报文头24个字节第二行报文头43
         	//m_Fixpt_TOTAL_Leng =17+((DWORD)(18+YC_NUM*10)* LoadDay);//文件头17个字节+节头+YC的个数
        	m_Fixpt_TOTAL_Leng = 24+43+((DWORD)(28+YC_NUM*11)* LoadDay);
		}
  ...
  else if(File_TYPE == 4)
  {
   // TOTAL_Leng = 12+soe_num * SOE_RECORD_LEN;//报文头9个字节
	TOTAL_Leng = 14 + 33+ soe_num * SOE_RECORD_LEN;//报文头9个字节
  ...
  else if(File_TYPE == 5)
  {
    ...
    if(FLogInfo[FLOG_CS] != (FLogInfo[FLOG_TOTALNUM] + FLogInfo[FLOG_NEW] + FLogInfo[FLOG_OLD]))//+FLoadInfo[FLOAD_DAY]) || FLoadInfo[FLOAD_TOTALNUM] > FLASH_LOAD_MAXNUM || FLoadInfo[FLOAD_NEW] > FLASH_LOAD_MAXNUM)//如果FLASH地址不在负荷记录保存区域内
      ...
      mwavelog_New = FLogInfo[FLOG_NEW];
    //mLog_TOTAL_Leng = 9+mwavelog_total * FLASH_PLOAD_LEN;//报文头9个字节
	mLog_TOTAL_Leng = 15 + 31+ (long)mwavelog_total * FLASH_PLOAD_LEN;//报文头9个字节
...
void CBJ101S::ulog_directory_confirm(WORD InfoAddr,DWORD Directory_ID)
{
  ...
  m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ] =0;
  mLog_TOTAL_Leng = 15 + 31+ (long)mwavelog_total * FLASH_PLOAD_LEN;//9+mwavelog_total * FLASH_PLOAD_LEN;//报文头9个字节
  m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ]  = mLog_TOTAL_Leng;
  m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ]  = mLog_TOTAL_Leng>>8;
  m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ]  = mLog_TOTAL_Leng>>16;
  m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ]  = mLog_TOTAL_Leng>>24;
...
void CBJ101S::soe_directory_confirm(WORD InfoAddr,DWORD Directory_ID)
{
  ...
  TOTAL_Leng = 14 + 33+ soe_num * SOE_RECORD_LEN;//12+soe_num * SOE_RECORD_LEN;//报文头9个字节
...
void CBJ101S::fixpt_directory_confirm(WORD InfoAddr,DWORD Directory_ID,BYTE call_sign,DWORD start_minute_time,DWORD start_date_time,DWORD end_minute_time,DWORD end_date_time)
{
  ...
  if((FLoadInfo[FLOAD_CS] != FLoadInfo[FLOAD_TOTALNUM] + FLoadInfo[FLOAD_NEW] + FLoadInfo[FLOAD_OLD]+FLoadInfo[FLOAD_DAY]) || FLoadInfo[FLOAD_TOTALNUM] > FLASH_LOAD_MAXNUM || FLoadInfo[FLOAD_NEW] > FLASH_LOAD_MAXNUM)//如果FLASH地址不在负荷记录保存区域内
    ...
    else
    {
      ...
      for( BYTE i=0;(m_SendFixNum < m_wavefix_total)&&(i< 5);i++,m_SendFixNum++)
      {
          fixpt_sign = OFF;
        ...
        if(call_sign ==1)
        {
          ...
          if((date_time >= start_date_time) &&(date_time <=end_date_time)) 
          {
            ...
            m_fixpt_num++;
                    ...
                    if((fixpt_sign ==OFF) &&(m_SendFixNum < m_wavefix_total)&&(i==4))
                          i =0; 
        }
        ...
        //Sst26vf064b_Read(FLoadAddrh,byLoadDa,FLASH_PLOAD_LEN);
        fix_num++;
        sprintf((char *)ch,"fixpt%04d%02d%02d.msg",(byLoadDa[2]+2000),byLoadDa[3],byLoadDa[4]);//strlen(pdat_name),pdat_name,
        //sprintf((char *)ch,"FIX");
              m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ] = strlen(ch);
            for(BYTE j = 0; j < strlen(ch); j++)
            m_SendBuf.pBuf[ m_SendBuf.wWritePtr++ ] = ch[j];
        ...
        for(BYTE m = 0; m < g_ucYcTransNum; m++)   //计算需要上传的遥测值，按照具体情况进行换算
		  ...
		  if(m_guiyuepara.yctype == M_ME_NC)  //短浮点数
		 {
		 	...
		 	m_Fixpt_SECTION_Leng = 24 + 43+28+YC_NUM*14;//第一行报文头24个字节第二行报文头43
         	//m_Fixpt_TOTAL_Leng =17+((DWORD)(18+YC_NUM*10)* LoadDay);//文件头17个字节+节头+YC的个数
        	m_Fixpt_TOTAL_Leng = 24+43+((DWORD)(28+YC_NUM*14)* (LoadDay[m_SendFixNum]));
		}
		else 
		{
			m_Fixpt_SECTION_Leng = 24 + 43+28+YC_NUM*11;//第一行报文头24个字节第二行报文头43
         	//m_Fixpt_TOTAL_Leng =17+((DWORD)(18+YC_NUM*10)* LoadDay);//文件头17个字节+节头+YC的个数
        	m_Fixpt_TOTAL_Leng = 24+43+((DWORD)(28+YC_NUM*11)* (LoadDay[m_SendFixNum]));
		}
         //m_Fixpt_SECTION_Leng = 17+18+YC_NUM*10;
...
void CBJ101S::lubo_directory_confirm(WORD InfoAddr,DWORD Directory_ID,BYTE call_sign,DWORD start_minute_time,DWORD start_date_time,DWORD end_minute_time,DWORD end_date_time)
{
  ...
  if(g_gRunPara[RP_LBSTORAGE_TIME] != RP_LBSTORAGE_INVALID)//为0XFFFF时表示不受该参数的限制
    {
