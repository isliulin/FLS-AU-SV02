...
//T-ESAM
#define CAT_SELECT_TESAM     P1OUT &= ~BIT5  //加密芯片片选信号
#define CAT_UNSELECT_TESAM   P1OUT |= BIT5
...
void SendBytesToEncChip(unsigned char *pdata,unsigned char num)
{
    ...
    CAT_SELECT_TESAM;
    delayms(1);
    CAT_SPIWriteBuf(0x55);  
    for(unsigned char i = 0; i < num; i++)
    {
        CAT_SPIWriteBuf(pdata[i]);
    }
    CAT_SPIWriteBuf(CheckLrc(pdata,num));  //写校验
    CAT_UNSELECT_TESAM;
    delayms(200);
    //return true;
}
...
int SendCmdToEncChip(const char *sCmd,BYTE *EncDate,WORD DataLen)
{
    ...
    if((DataLen != 0) && (EncDate != NULL))
    {
        ...
        {
            CAT_SPIWriteBuf(EncDate[i]);
            Check ^= EncDate[i];
        }
        count += DataLen;
    } 
    Check = ~Check;
    CAT_SPIWriteBuf(Check);
    delayms(1);
    CAT_UNSELECT_TESAM;
    delayms(200);
